...
def inference(args, wavlm_model, audio, sample_fn, model, n_frames=0, smoothing=False, SG_filter=False, minibatch=False, skip_timesteps=0, n_seed=8, style=None, seed=123456, save_dir=None):
...
def main(args, save_dir, model_path, audio_path=None, mfcc_path=None, audiowavlm_path=None, max_len=0):
    ...
    filename = os.path.basename(audiowavlm_path)
    style_key = filename.split('_')[1]
    style = style2onehot[style_key]
    # style = [0, 0, 1, 0, 0, 0]
    # style = style2onehot['Neutral']
    print(style)

    wavlm_model = wavlm_init(mydevice)
    inference(args, wavlm_model, mfcc, sample_fn, model, n_frames=max_len, smoothing=True, SG_filter=True, minibatch=True, skip_timesteps=0, style=style, seed=123456, save_dir=save_dir)      # style2onehot['Happy']


def main_wrapper(config, wav_file, save_dir='sample_dir/'+str(datetime.now().strftime('%Y%m%d_%H%M%S'))):
    import torch
    global mydevice
    ...
    config.audiowavlm_path = wav_file
    main(config, save_dir, config.model_path, audiowavlm_path=wav_file)
